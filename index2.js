...
let [homeSlider, homeCompany, homeCeo, homeServices, homeStats, homeServicesOther, homeTestimonials, homePublications, homeNews, homePartners, prefooter] = Array.from({length: 11}, (_, i) => {return makeSection()});


setChildren(main, [
    homeSlider, homeCompany, homeCeo, homeServices, 
    homeStats, homeServicesOther, homeTestimonials, homePublications, homeNews,
    homePartners, prefooter,
]);
...
let backgroundStripe = () => {
    let output = createContainer('div', {
        flex: 1,
        border: '2px solid purple',
    });
    let backgroundTop = createContainer('div', {
        width: '100%',
        height: '50%',
        backgroundColor: 'white',
    });
    let backgroundBot = createContainer('div', {
        width: '100%',
        height: '50%',
        backgroundColor: '#03153e',
    });
    setChildren(output, [backgroundTop, backgroundBot]);
    return output;
}

let createPrefooter = (section) => {
    section.style.position = 'relative';
    let prefooterContainer = createContainer('div', {
        display: 'flex',
    });
    let prefooterWrapper = createContainer('div', {
        display: 'flex',
        flexWrap: 'wrap',
    ...
    setChildren(prefooterWrapper, [backgroundStripe(), prefooterLeft, prefooterMid, prefooterRight, backgroundStripe()]);
    //setChildren(prefooterContainer, [prefooterWrapper]);
    setChildren(section, [prefooterWrapper]);
}
createPrefooter(prefooter);
...
let createFooter = (footer) => {
    ...
    setChildren(footer, [infoBar, copyright]);
};
